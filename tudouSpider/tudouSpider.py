...
Website = 'tudou'
type = sys.getfilesystemencoding()  # get wordtype of current machine with Windows OS
...
class BTSpider:
        ...
        if self.type == 'video':
            ...
            while choosed != 'q':
                while not isNum(choosed):
                    choosed = raw_input('need a number or enter \'q\' please: ')
                    if choosed == 'q': return
    ...
    def getReallink(self, dramaTitle, link, f):
        ...
        linkmatch = re.compile(r'http://k.youku.com/player\S{1,400}(?=")')  # 正则表达式
